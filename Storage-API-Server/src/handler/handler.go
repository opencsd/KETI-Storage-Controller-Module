	...
	if result, err := storagestruct.INFLUX_CLIENT.Query(q); err == nil && result.Error() == nil {
		for _, row := range result.Results[0].Series {
			for _, value := range row.Values {
				...
				nodeMetric.MemoryTotal = parseFloat(value[4])
				nodeMetric.MemoryUsed = parseFloat(value[5])
				nodeMetric.MemoryUtilization = parseFloat(value[6])
				nodeMetric.NetworkBandwidth = parseFloat(value[7])
				nodeMetric.NetworkRxData = parseFloat(value[8])
				nodeMetric.NetworkTxData = parseFloat(value[9])
				nodeMetric.NodeName = fmt.Sprintf("%v", value[10])
				nodeMetric.PowerUsed = parseFloat(value[11])
